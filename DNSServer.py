...
import dns.rdata
import dns.rrset # Added for RRset
import socket
import threading
import signal
import os
...
dns_records = {
    ...
    'safebank.com.': {
        dns.rdatatype.A: '192.168.1.102', # [cite: 63]
    },
    'google.com.': {
        dns.rdatatype.A: '192.168.1.103', # [cite: 64]
    },
    'legitsite.com.': {
        dns.rdatatype.A: '192.168.1.104', # [cite: 64]
    },
    'yahoo.com.': {
        dns.rdatatype.A: '192.168.1.105', # [cite: 65, 67]
    },
    'nyu.edu.': {
...
def run_dns_server():
    ...
    server_socket.bind(('127.0.0.1', 53)) #  Bind to localhost on standard DNS port 53

    while True:
        try:
            # Wait for incoming DNS requests
            data, addr = server_socket.recvfrom(1024) # [cite: 80]
            ...
            response = dns.message.make_response(request) # [cite: 82, 83]
            ...
            if qname in dns_records and qtype in dns_records[qname]: # [cite: 85]
            ...
            response.flags |= 1 << 10 #  Set Authoritative Answer (AA) flag
